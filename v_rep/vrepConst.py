...
# Whole model is invisible independent of local visibility settings
...
# a UI button slider etc. changed (due to a user's action). aux[0]=UI handle aux[1]=button handle aux[2]=button attributes aux[3]=slider position (if slider)
...
# a key was pressed while the focus was on a page (aux[0]=key aux[1]=ctrl and shift key state)
...
# called when simHandleModule in Lua is called with argument false
...
# Called once every main application loop pass. auxiliaryData[0] contains event flags of events that happened since last time
...
sim_message_eventcallback_abouttoundo = 0x118
# the undo button was hit and a previous state restored
sim_message_eventcallback_undoperformed = 0x119
# the redo button was hit and a future state is about to be restored
sim_message_eventcallback_abouttoredo = 0x11a
# the redo button was hit and a future state restored
sim_message_eventcallback_redoperformed = 0x11b
# scipt icon was double clicked.  (aux[0]=object handle associated with script set replyData[0] to 1 if script should not be opened)
sim_message_eventcallback_scripticondblclick = 0x11c
...
# a key was pressed while the focus was on a page (aux[0]=key aux[1]=ctrl and shift key state)
sim_message_eventcallback_keypress = 0x120
# called when simHandleModule in Lua is called with argument true
sim_message_eventcallback_modulehandleinsensingpart = 0x121
# called just before the scene is rendered
sim_message_eventcallback_renderingpass = 0x122
# called when a banner was clicked (aux[0]=banner ID)
sim_message_eventcallback_bannerclicked = 0x123
# auxiliaryData[0] indicates the handle of the item auxiliaryData[1] indicates the state of the item
sim_message_eventcallback_menuitemselected = 0x124
# aux[0]=refresh degree (0=light 1=medium 2=full)
sim_message_eventcallback_refreshdialogs = 0x125
...
sim_message_eventcallback_rmlposition = 0x12b
# the command simRMLVelocity was called. The appropriate plugin should handle the call
sim_message_eventcallback_rmlvelocity = 0x12c
...
sim_simulation_stopped = 0x00
# Simulation is paused
sim_simulation_paused = 0x08
# Simulation is advancing
sim_simulation_advancing = 0x10
# First simulation pass (1x)
sim_simulation_advancing_firstafterstop = sim_simulation_advancing | 0x00
# Normal simulation pass (>=1x)
sim_simulation_advancing_running = sim_simulation_advancing | 0x01
# reserved                                    =sim_simulation_advancing|0x02
# Last simulation pass before pause (1x)
sim_simulation_advancing_lastbeforepause = sim_simulation_advancing | 0x03
# First simulation pass after pause (1x)
sim_simulation_advancing_firstafterpause = sim_simulation_advancing | 0x04
# "Trying to stop" simulation pass (>=1x)
sim_simulation_advancing_abouttostop = sim_simulation_advancing | 0x05
...
# can be combined with one of above values. Only with this flag can the position of the related UI be set just after dialog creation
...
# 6 values per point (3 for triangle position 3 for triangle normal vector) (triangle size in meters)
sim_drawing_trianglepoints = 3
# 6 values per point (3 for quad position 3 for quad normal vector) (quad size in meters)
sim_drawing_quadpoints = 4
# 6 values per point (3 for disc position 3 for disc normal vector) (disc size in meters)
sim_drawing_discpoints = 5
# 6 values per point (3 for cube position 3 for cube normal vector) (cube size in meters)
sim_drawing_cubepoints = 6
# 3 values per point (sphere size in meters)
sim_drawing_spherepoints = 7

# following can be or-combined
# +3 values per item (each item has its own ambient color (rgb values)).
sim_drawing_itemcolors = 0x00020
# Mutually exclusive with sim_drawing_vertexcolors
# +3 values per vertex (each vertex has its own ambient color (rgb values). Only for sim_drawing_lines (+6) and for sim_drawing_triangles(+9)). Mutually exclusive with sim_drawing_itemcolors
sim_drawing_vertexcolors = 0x00040
# +1 value per item (each item has its own size). Not for sim_drawing_triangles
sim_drawing_itemsizes = 0x00080
# back faces are not displayed for all items
sim_drawing_backfaceculling = 0x00100
sim_drawing_wireframe = 0x00200  # all items displayed in wireframe
# all items are tagged as paint (for additinal processing at a later stage)
sim_drawing_painttag = 0x00400
# if the object is associated with a scene object then it follows that visibility otherwise it is always visible
sim_drawing_followparentvisibility = 0x00800
# if the max item count was reached then the first items are overwritten.
sim_drawing_cyclic = 0x01000
# the drawing object will be 50% transparent
sim_drawing_50percenttransparency = 0x02000
# the drawing object will be 25% transparent
sim_drawing_25percenttransparency = 0x04000
# the drawing object will be 12.5% transparent
sim_drawing_12percenttransparency = 0x08000
...
sim_drawing_emissioncolor = 0x10000
# Only for trianglepoints quadpoints discpoints and cubepoints. If specified the normal verctor is calculated to face the camera (each item data requires 3 values less)
sim_drawing_facingcamera = 0x20000
# When specified objects are always drawn on top of "regular objects"
sim_drawing_overlay = 0x40000
# +1 value per item (each item has its own transparency value (0-1)). Not compatible with sim_drawing_vertexcolors
sim_drawing_itemtransparency = 0x80000
...
# If specified the banner will always face the camera by rotating around the banner's vertical axis (y-axis)
...
# If specified the banner will always appear in the same size. In that case size represents the character height in pixels
sim_banner_keepsamesize = 0x00400
# If specified a fixed-size bitmap font is used. The text will also always fully face the camera and be right
...
# 6 values per point (pt1 and pt2. Pt1 is start position pt2-pt1 is the initial velocity vector). i
sim_particle_points1 = 0
# Point is 1 pixel big. Only appearance is a point internally handled as a perfect sphere
# 6 values per point. Point is 2 pixel big. Only appearance is a point internally handled as a perfect sphere
sim_particle_points2 = 1
# 6 values per point. Point is 4 pixel big. Only appearance is a point internally handled as a perfect sphere
sim_particle_points4 = 2
# 6 values per sphere. Only appearance is rough. Internally a perfect sphere
sim_particle_roughspheres = 3
sim_particle_spheres = 4    # 6 values per sphere. Internally a perfect sphere


# following can be or-combined
# the particles are respondable against shapes (against all objects that have at least one bit 1-4 activated in the global respondable mask)
sim_particle_respondable1to4 = 0x0020
# the particles are respondable against shapes (against all objects that have at least one bit 5-8 activated in the global respondable mask)
...
# the particles ignore the effect of gravity. Not compatible with sim_particle_water
sim_particle_ignoresgravity = 0x0100
sim_particle_invisible = 0x0200  # the particles are invisible
# +1 value per particle (each particle can have a different size)
sim_particle_itemsizes = 0x0400
# +1 value per particle (each particle can have a different density)
sim_particle_itemdensities = 0x0800
# +3 values per particle (each particle can have a different color)
...
# the particles are water particles (no weight in the water (i.e. when z<0)). Not compatible with sim_particle_ignoresgravity
sim_particle_water = 0x8000
# The particles can be seen by vision sensors (sim_particle_invisible must not be set)
...
# Integer parameters
# Check sim_api_errormessage_... constants above for valid values
sim_intparam_error_report_mode = 0
sim_intparam_program_version = 1  # e.g Version 2.1.4 --> 20104. Can only be read
# do not use anymore (always returns 1 since V-REP 2.5.11)
...
# is initialized at sim_intparam_server_port_start
...
SIMX_HEADER_SIZE = 18
# 1 simxUShort. Generated by the client or server. The CRC for the message
simx_headeroffset_crc = 0
# 1 byte. Generated by the client or server. The version of the remote API software
simx_headeroffset_version = 2
# 1 simxInt. Generated by the client (and used in a reply by the server)
simx_headeroffset_message_id = 3
# 1 simxInt. Client time stamp generated by the client (and sent back by the server)
simx_headeroffset_client_time = 7
# 1 simxInt. Generated by the server when a reply is generated. The server timestamp
simx_headeroffset_server_time = 11
# 1 simxUShort. Generated by the server. A unique ID identifying the scene currently displayed
simx_headeroffset_scene_id = 15
# 1 byte. Generated by the server. Bit coded 0 set --> simulation not stopped 1 set --> simulation paused 2 set --> real-time switch on 3-5 edit mode type (0=no edit mode 1=triangle 2=vertex 3=edge 4=path 5=UI)
simx_headeroffset_server_state = 17

# Remote API command header
SIMX_SUBHEADER_SIZE = 26
# 1 simxInt. Generated by the client or server. The buffer size of the command.
simx_cmdheaderoffset_mem_size = 0
# 1 simxInt. Generated by the client or server. The full buffer size of the command (applies to split chunks).
simx_cmdheaderoffset_full_mem_size = 4
# 1 simxUShort. Generated by the client or server. The amount of data that is part of the command identification.
simx_cmdheaderoffset_pdata_offset0 = 8
# 1 simxInt. Generated by the client or server. The amount of shift of the pure data buffer (applies to split chunks).
simx_cmdheaderoffset_pdata_offset1 = 10
# 1 simxInt. Generated by the client (and used in a reply by the server). The command combined with the operation mode of the command.
simx_cmdheaderoffset_cmd = 14
# 1 simxUShort. Generated by the client or server. The amount of delay in ms of a continuous command or the max. pure data size to send at once (applies to split commands).
simx_cmdheaderoffset_delay_or_split = 18
# 1 simxInt. Generated by the server. The simulation time (in ms) when the command was executed (or 0 if simulation is not running)
simx_cmdheaderoffset_sim_time = 20
# 1 byte. Generated by the server. (1 bit 0 is set --> error in function execution on server side). The client writes bit 1 if command cannot be overwritten
simx_cmdheaderoffset_status = 24
simx_cmdheaderoffset_reserved = 25    # 1 byte. Not yet used


# Regular operation modes
# sends command as one chunk. Reply will also come as one chunk. Doesn't wait for the reply.
simx_opmode_oneshot = 0x000000
# sends command as one chunk. Reply will also come as one chunk. Waits for the reply (_REPLY_WAIT_TIMEOUT_IN_MS is the timeout).
simx_opmode_blocking = 0x010000
# sends command as one chunk. Reply will also come as one chunk. Waits for the reply (_REPLY_WAIT_TIMEOUT_IN_MS is the timeout).
simx_opmode_oneshot_wait = 0x010000
simx_opmode_continuous = 0x020000
# sends command as one chunk. Command will be stored on the server and always executed
...
# sends command as several chunks (max chunk size is x bytes where x can be _MIN_SPLIT_AMOUNT_IN_BYTES-65535. Just add x to opmode_oneshot_split). Reply will also come as several chunks. Doesn't wait for the reply.
simx_opmode_oneshot_split = 0x030000
simx_opmode_continuous_split = 0x040000
# sends command as several chunks (max chunk size is x bytes where x can be _MIN_SPLIT_AMOUNT_IN_BYTES-65535. Just add x to opmode_continuous_split). Command will be stored on the server and always executed. A reply will be sent continuously each time as several chunks. Doesn't wait for the reply.
simx_opmode_streaming_split = 0x040000

# Special operation modes
# removes and cancels all commands stored on the client or server side (also continuous commands)
simx_opmode_discontinue = 0x050000
# doesn't send anything but checks if a reply for the given command is available in the input buffer (i.e. previously received from the server)
simx_opmode_buffer = 0x060000
# doesn't send anything and doesn't return any specific value. It just erases a similar command reply in the inbox (to free some memory)
...
# command reply not received in time for opmode_oneshot_wait operation mode
simx_return_timeout_flag = 0x000002
# command doesn't support the specified operation mode
simx_return_illegal_opmode_flag = 0x000004
# command caused an error on the server side
simx_return_remote_error_flag = 0x000008
# previous similar command not yet fully processed (applies to opmode_oneshot_split operation modes)
simx_return_split_progress_flag = 0x000010
# command caused an error on the client side
simx_return_local_error_flag = 0x000020
